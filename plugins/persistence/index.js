/**
 * Persistence plugin for goliath.
 *
...
export async function register(app, ctx, pluginConfig = {}) {
  ...
  // Resolve profileDir: env var > plugin config > global config default (~/.goliath/profiles)
  const profileDir = process.env.GOLIATH_PROFILE_DIR || pluginConfig.profileDir || config.profileDir;
  if (!profileDir) {
  ...
  events.on('session:creating', async ({ userId, contextOptions }) => {
    ...
    if (storageStatePath && contextOptions.storageState === undefined) {
