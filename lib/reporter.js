// lib/reporter.js -- Crash/hang reporter for goliath
// Sends opt-in crash/hang reports with paranoid anonymization. No env reads here.
...
export function stackSignature(type, error) {
  ...
  for (const frame of frames) {
    ...
    if (trimmed.startsWith('at ') && !trimmed.includes('node_modules') && !trimmed.includes('node:internal')) {
      ...
      const loc = openParen >= 0 && closeParen > openParen
...
function fnv1a(str) {
  let hash = 0x811c9dc5;
  const bounded = String(str).slice(0, 1024);
  for (let i = 0; i < 1024; i++) {
    const charCode = bounded.charCodeAt(i);
    if (Number.isNaN(charCode)) break;
    hash ^= charCode;
...
// No endpoint is built in. Operators must explicitly configure a trusted HTTPS relay.
let _relayUrl = '';
...
function formatIssueBody(type, detail) {
  const sections = [
    '> Auto-reported by goliath-crash-reporter. All data is anonymized.',
    '',
    '## Environment',
 ...
 * @param {string}  config.crashReportUrl       - explicit HTTPS relay endpoint
...
export function createReporter(config) {
  _relayUrl = typeof config.crashReportUrl === 'string'
    ? config.crashReportUrl.trim()
    : '';

  const enabled = config.crashReportEnabled === true && /^https:\/\//i.test(_relayUrl);
  const rateLimiters = {
