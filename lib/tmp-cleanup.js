...
// Firefox temp profile directories created by Playwright/Goliath
const FIREFOX_PROFILE_PATTERN = /^playwright_firefoxdev_profile-/;
// Goliath also creates these
const GOLIATH_TMP_PATTERN = /^goliath[-_]/;
...
/**
 * Clean up stale Firefox/Goliath temp profile directories.
...
export function cleanupStaleFirefoxProfiles({ tmpDir, minAgeMs = 2 * 60 * 1000, now = Date.now() } = {}) {
  ...
  for (const name of entries) {
    if (!FIREFOX_PROFILE_PATTERN.test(name) && !GOLIATH_TMP_PATTERN.test(name)) continue;
    result.scanned++;
