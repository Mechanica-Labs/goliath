...
test('paused handoffs keep sessions alive until resumed or cancelled', () => {
  const tabState = { handoff: { handoffId: 'h1', status: 'paused' } };
  const session = { activeOperations: 0, tabGroups: new Map([['default', new Map([['tab', tabState]])]]) };
  expect(sessionHasPausedHandoff(session)).toBe(true);
  expect(sessionCanBeReaped(session)).toBe(false);
  tabState.handoff.status = 'resumed';
  expect(sessionHasPausedHandoff(session)).toBe(false);
  expect(sessionCanBeReaped(session)).toBe(true);
});
