...
export async function humanizedClick(page, locator, state, options = {}) {
  ...
  return { mode: 'humanized', profile: rt.profileName, pathPoints: movement.points, visualized: options.visualize === true };
}
...
export async function humanizedPressAndHold(page, locator, state, options = {}) {
  ...
  await page.mouse.up();
  rt.record('pointerup', { button: 'left' });
  return { mode: 'humanized', profile: rt.profileName, pathPoints: movement.points, holdMs: elapsed, visualized: options.visualize === true };
}
