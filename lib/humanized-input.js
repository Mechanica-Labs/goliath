...
function runtime(options = {}) {
  ...
  return { random, sleep, record, dispatch, profile: PROFILES[profileName], profileName };
}
...
// Runs inside the page: count click events that reach the element itself.
function armClickProbe(el) {
  const probe = { clicks: 0, handler: null };
  probe.handler = () => { probe.clicks += 1; };
  el.addEventListener('click', probe.handler, true);
  el.__goliathClickProbe = probe;
}

function readClickProbe(el) {
  const probe = el.__goliathClickProbe;
  if (!probe) return null;
  el.removeEventListener('click', probe.handler, true);
  delete el.__goliathClickProbe;
  return probe.clicks;
}
...
async function verifyPointerOverTarget(page, locator, state, target, box, rt) {
  let hit;
  try {
    hit = await locator.evaluate(hitTestInPage, { x: target.x, y: target.y });
  } catch {
    return { checked: false, reason: 'evaluate_failed' };
  }
  if (!hit.checked || hit.hit) return hit;
  // One corrective re-aim: the layout may have shifted while the pointer travelled.
  const freshBox = await locator.boundingBox();
  ...
  target.y = retarget.y;
  try {
    const second = await locator.evaluate(hitTestInPage, { x: target.x, y: target.y });
...
export async function humanizedClick(page, locator, state, options = {}) {
  ...
  let delivered = null;
  try {
    handle = await locator.elementHandle({ timeout: 3000 });
    await handle.evaluate(armClickProbe);
    delivered = false;
  } catch {
    handle = null;
  }
  ...
  try {
    ...
    if (handle) {
      try {
        const clicks = await handle.evaluate(readClickProbe);
        // null: the element navigated away or was torn down before we could read
        // the probe, which is itself a sign the click did something.
        delivered = clicks === null ? null : clicks > 0;
      } catch {
        delivered = null;
      }
    }
  ...
  if (delivered === false) {
    throw new HumanizedInputError(
      'The pointer press did not deliver a click event to the target element',
      'click_not_delivered',
      { hit, delivered, hint: 'The element was replaced, covered, or moved between pointer-down and pointer-up. Call snapshot and retry, or send humanized:false to use the direct click path.' },
    );
  }
