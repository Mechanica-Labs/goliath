...
export function writeJournal(paths, journal) {
  const value = {
    ...
    records: structuredClone(journal.records || []),
    appliedClients: [...(journal.appliedClients || [])],
    priorState: journal.priorState == null ? null : structuredClone(journal.priorState),
    targetState: journal.targetState == null ? null : structuredClone(journal.targetState),
...
export function readJournal(paths) {
  ...
  if (journal?.schemaVersion !== 2 || !journal.runId || !['install', 'uninstall'].includes(journal.operation)
      || !Array.isArray(journal.records) || !Array.isArray(journal.appliedClients)) {
    throw new Error('harness installer transaction has an invalid schema');
  }
  for (const record of journal.records) {
    if (!record?.client || !Object.hasOwn(record, 'beforeEntry') || !Object.hasOwn(record, 'afterEntry')) {
      throw new Error('harness installer transaction has an invalid record');
    }
  }
  return journal;
}
...
export function removeHarnessState(paths) {
