...
test('humanizedOptions accepts booleans and bounded profiles', () => {
  ...
  expect(humanizedOptions(true)).toEqual({ enabled: true, profile: 'balanced', visualize: false });
  expect(humanizedOptions({ profile: 'fast' })).toEqual({ enabled: true, profile: 'fast', visualize: false });
  expect(humanizedOptions({ profile: 'deliberate', visualize: true })).toEqual({ enabled: true, profile: 'deliberate', visualize: true });
...
test('humanizedClick installs the live pointer visualizer when requested', async () => {
  const page = mockPage();
  const frame = { evaluate: jest.fn(async () => {}) };
  page.frames = () => [frame];
  const locator = { boundingBox: async () => ({ x: 40, y: 60, width: 100, height: 40 }) };

  const result = await humanizedClick(page, locator, {}, {
    visualize: true,
    random: () => 0.5,
    sleep: async () => {},
  });
