 ...
 *     session:policy:updated  { userId, policy }               -- runtime policy set or replaced
 *     session:policy:cleared  { userId }                       -- runtime policy removed
 ...
 *     tab:approval_required   { userId, tabId, action, kind, category, risk, element, domain, source }
 *
 *   DOWNLOADS
...
const CONFIG_PATH = path.join(ROOT_DIR, 'goliath.config.json');

/**
 * Read plugin configuration from goliath.config.json.
...
export async function loadPlugins(app, ctx) {
  ...
  const enabledOverrides = new Set(ctx.config.pluginEnableOverrides || []);
  ...
  for (const entry of entries) {
    ...
    // If goliath.config.json specifies a plugins list, only load those
    if (allowList && !allowList.includes(name) && !enabledOverrides.has(name)) {
      ctx.log('debug', `plugin "${name}" not in goliath.config.json plugins list, skipping`);
      continue;
    }
