import { planAction, validateContract, validatePostconditions, verifyPostconditions } from './action-contracts.js';
...
test('node_exists applies all supplied fields to the same node', () => {
  const verification = verifyPostconditions([
    { kind: 'node_exists', nodeId: 'wanted', name: 'WRONG', role: 'button' },
  ], {
    snapshot: { nodes: [{ id: 'wanted', name: 'Actual', role: 'button' }] },
  });
  expect(verification).toMatchObject({ verified: false });
});

test('postconditions reject empty, unknown, and malformed conditions', () => {
  expect(validatePostconditions([])).toMatchObject({ ok: false });
  expect(validatePostconditions([{ kind: 'text_contains', text: '' }])).toMatchObject({ ok: false });
  expect(validatePostconditions([{ kind: 'node_exists' }])).toMatchObject({ ok: false });
  expect(validatePostconditions([{ kind: 'node_exists', nodeId: 'n1', extra: true }])).toMatchObject({ ok: false });
  expect(validatePostconditions([{ kind: 'made_up', text: 'x' }])).toMatchObject({ ok: false });
  expect(() => verifyPostconditions([{ kind: 'text_contains', text: '' }], { snapshot: { nodes: [] } }))
