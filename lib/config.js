...
import os from 'os';
import { parseDangerousActionMode } from './dangerous-actions.js';
...
/**
 * Parse named egress profiles for anti-bot walls.
 *
 * Accepts inline JSON (GOLIATH_EGRESS_PROFILES) or a file path
 * (GOLIATH_EGRESS_PROFILES_FILE). When neither is set, the existing PROXY_*
 * configuration becomes a single profile so current deployments keep working
 * and gain a recorded network class.
 */
function parseEgressProfiles(profilesEnv, fileEnv, proxyDefaults = {}) {
  const readJson = (text) => {
    if (!text) return null;
    try { return JSON.parse(text); } catch { return null; }
  };
  let parsed = readJson(profilesEnv);
  if (!parsed && fileEnv) {
    try { parsed = readJson(fs.readFileSync(resolve(fileEnv), 'utf8')); } catch { parsed = null; }
  }
  if (Array.isArray(parsed) && parsed.length) return parsed;
  const server = proxyDefaults.backconnectHost
    ? `${proxyDefaults.backconnectHost}:${proxyDefaults.backconnectPort}`
    : (proxyDefaults.host ? `${proxyDefaults.host}:${proxyDefaults.port || ''}`.replace(/:$/, '') : '');
  if (!server) return [];
  return [{
    name: proxyDefaults.name || 'default',
    class: proxyDefaults.class || 'unknown',
    server,
    username: proxyDefaults.username || null,
    password: proxyDefaults.password || null,
    provider: proxyDefaults.providerName || null,
    country: proxyDefaults.country || null,
    city: proxyDefaults.city || null,
  }];
}
...
function loadConfig() {
  ...
  return {
    ...
    workspaceDir: (process.env.GOLIATH_WORKSPACE || '').trim(),
    ...
    // Brake for click/type/hands on controls that send, pay, publish, delete, sign, etc.
    // 'confirm' (default) refuses them without confirm:true; 'annotate' only labels; 'off' disables.
    dangerousActionsMode: parseDangerousActionMode(process.env.GOLIATH_DANGEROUS_ACTIONS),
    ...
    pluginEnableOverrides: process.env.ENABLE_VNC === '1' ? ['vnc'] : [],
    ...
    // Anti-bot walls: detection, typed verdicts, and fleet egress profiles.
    antibot: {
      detectOnNavigation: process.env.GOLIATH_ANTIBOT_DETECT !== '0',
      driveChallenges: process.env.GOLIATH_ANTIBOT_DRIVE !== '0',
      // Vendors inject their interstitial script after the document lands, so a
      // challenge-status document is re-checked once after this settle window.
      settleMs: parseInt(process.env.GOLIATH_ANTIBOT_SETTLE_MS || '2500', 10),
      autoRotate: process.env.GOLIATH_EGRESS_AUTOROTATE !== '0',
      maxRotations: parseInt(process.env.GOLIATH_EGRESS_MAX_ROTATIONS || '1', 10),
      stateDir: process.env.GOLIATH_EGRESS_STATE_DIR || join(os.homedir(), '.goliath'),
      classHint: process.env.GOLIATH_EGRESS_CLASS || '',
      profiles: parseEgressProfiles(process.env.GOLIATH_EGRESS_PROFILES, process.env.GOLIATH_EGRESS_PROFILES_FILE, {
        name: process.env.GOLIATH_EGRESS_PROFILE_NAME || 'default',
        class: process.env.GOLIATH_EGRESS_CLASS || '',
        providerName: process.env.PROXY_PROVIDER || 'decodo',
        host: process.env.PROXY_HOST || '',
        port: process.env.PROXY_PORT || '',
        backconnectHost: process.env.PROXY_BACKCONNECT_HOST || '',
    ...
    serverEnv: {
      ...
      GOLIATH_SCREENSHOTS_DIR: process.env.GOLIATH_SCREENSHOTS_DIR,
      GOLIATH_WORKSPACE: process.env.GOLIATH_WORKSPACE,
      GOLIATH_DANGEROUS_ACTIONS: process.env.GOLIATH_DANGEROUS_ACTIONS,
      ...
      GOLIATH_ANTIBOT_DETECT: process.env.GOLIATH_ANTIBOT_DETECT,
      GOLIATH_ANTIBOT_DRIVE: process.env.GOLIATH_ANTIBOT_DRIVE,
      GOLIATH_ANTIBOT_SETTLE_MS: process.env.GOLIATH_ANTIBOT_SETTLE_MS,
      GOLIATH_EGRESS_AUTOROTATE: process.env.GOLIATH_EGRESS_AUTOROTATE,
      GOLIATH_EGRESS_MAX_ROTATIONS: process.env.GOLIATH_EGRESS_MAX_ROTATIONS,
      GOLIATH_EGRESS_STATE_DIR: process.env.GOLIATH_EGRESS_STATE_DIR,
      GOLIATH_EGRESS_CLASS: process.env.GOLIATH_EGRESS_CLASS,
      GOLIATH_EGRESS_PROFILE_NAME: process.env.GOLIATH_EGRESS_PROFILE_NAME,
      GOLIATH_EGRESS_PROFILES: process.env.GOLIATH_EGRESS_PROFILES,
      GOLIATH_EGRESS_PROFILES_FILE: process.env.GOLIATH_EGRESS_PROFILES_FILE,
    },
