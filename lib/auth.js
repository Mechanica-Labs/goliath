 ...
 *   - Policy administration uses requireApiKey instead and never accepts that superkey.
...
/**
 * Require the dedicated operator API key when one is configured.
 *
 * This is intentionally stricter than requireAuth(): the shared access key is
 * not a superkey for policy administration. Without a dedicated API key, the
 * existing loopback/access-key behavior remains available for local setups.
 */
export function requireApiKey(config, options = {}) {
  if (!config.apiKey) return requireAuth(config, options);
  return function requireApiKeyCheck(req, res, next) {
    const auth = String(req.headers['authorization'] || '');
    const scheme = auth.slice(0, 7).toLowerCase();
    const token = scheme === 'bearer ' ? auth.slice(7).trim() : null;
    if (token && timingSafeCompare(token, config.apiKey)) return next();
    return res.status(403).json({ error: 'Forbidden' });
  };
}
 ...
 *   - /sessions/:userId/policy (only when GOLIATH_API_KEY is set -- has its own strict gate)
...
export function accessKeyMiddleware(config) {
  return function accessKeyCheck(req, res, next) {
    ...
    if (config.apiKey && /^\/sessions\/[^/]+\/policy$/.test(req.path)) return next();
