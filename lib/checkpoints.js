...
async function withNamedCheckpointLock(directory, owner, checkpointId, operation) {
  ...
  try {
    ...
    const stale = Boolean(stat && Date.now() - stat.mtimeMs > LOCK_STALE_MS);
    const message = stale
      ? `stale checkpoint lock found at ${lockPath}; verify no checkpoint operation is running, remove this exact lock file, and retry`
      : 'checkpoint operation already in progress';
  ...
  try {
    ...
    const currentOwner = await fs.readFile(lockPath, 'utf8')
      ...
      .catch(() => null);
    if (currentOwner?.token === token) await fs.unlink(lockPath).catch(() => {});
  }
}
