...
function userDir(baseDir, userId) {
  return path.join(path.resolve(baseDir), crypto.createHash('sha256').update(String(userId)).digest('hex').slice(0, 24));
}
...
function checkpointPath(baseDir, userId, checkpointId) {
  ...
  if (!safe) return null;
  return path.join(userDir(baseDir, userId), `${safe}.json`);
}
...
async function listCheckpoints(baseDir, userId) {
  ...
  for (const name of names) {
    ...
    const item = await readCheckpoint(baseDir, userId, name.slice(0, -5));
    if (item) output.push({
