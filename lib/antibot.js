...
export async function driveChallenge(page, wall, { humanizedClick, humanizedPressAndHold, state, options = {}, holdMs = 4200, waitMs = 5000 } = {}) {
  ...
  const frameUrl = wall.challengeFrame;
  const frames = typeof page.frames === 'function' ? page.frames() : [];
  const frame = frames.find(f => { try { return f.url() === frameUrl; } catch { return false; } }) || (typeof page.mainFrame === 'function' ? page.mainFrame() : null);
  if (!frame) return { attempted: false, outcome: 'unsupported', detail: 'challenge frame not reachable' };

  let target = null;
  for (const selector of DRIVE_TARGETS) {
    const locator = frame.locator(selector).first();
    const count = await locator.count().catch(() => 0);
    if (count > 0 && await locator.isVisible().catch(() => false)) { target = { selector, locator }; break; }
  }
  if (!target) return { attempted: false, outcome: 'unsupported', detail: 'no interactive element inside the challenge frame' };

  const before = await observeRequests(page, async () => {
    if (typeof humanizedPressAndHold === 'function' && holdMs > 0) {
      await humanizedPressAndHold(page, target.locator, state, { ...options, holdMs });
    } else {
