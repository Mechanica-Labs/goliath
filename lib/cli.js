...
function setupDirectories(config) {
  const directories = [config.cookiesDir, config.uploadsDir, config.profileDir, config.tracesDir, config.screenshotsDir];
...
function helpText() {
  return [
    ...
    '  install        Verify Goliath and configure detected agent harnesses',
    '  upgrade        Update harnesses managed by goliath install',
    '  uninstall      Remove only harness entries managed by goliath install',
...
export async function runCli(argv, io = { stdout: process.stdout, stderr: process.stderr }) {
  ...
  if (command === 'install' || command === 'upgrade' || command === 'uninstall') {
    const { HarnessInstallUsageError, parseHarnessInstallArgs } = await import('./install/options.js');
    const {
      installHarnesses,
      renderHarnessResult,
      uninstallHarnesses,
      upgradeHarnesses,
    } = await import('./install/orchestrator.js');
    let options;
    try {
      options = parseHarnessInstallArgs(argv.slice(1));
    } catch (error) {
      if (!(error instanceof HarnessInstallUsageError)) throw error;
      write(io.stderr, error.message);
      write(io.stderr, 'Run goliath --help for usage.');
      process.exitCode = error.exitCode;
      return;
    }
    const dependencies = { io: { stdout: io.stdout, stderr: io.stderr } };
    const result = command === 'install'
      ? await installHarnesses({ options, dependencies })
      : command === 'upgrade'
        ? await upgradeHarnesses({ options, dependencies })
        : await uninstallHarnesses({ options, dependencies });
    io.stdout.write(renderHarnessResult(result, options));
