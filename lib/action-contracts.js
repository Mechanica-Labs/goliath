...
import { classifyDangerousAction } from './dangerous-actions.js';
...
const HIGH_RISK_BRAKE = new Set(['financial', 'destructive', 'credential_change', 'legal_commitment']);

function classifyActionRisk(action, node) {
  const description = `${action?.kind || ''} ${action?.intent || ''} ${node?.name || ''}`;
  // Share the fast-path brake's vocabulary so the contract flow is never looser than /click.
  const brake = classifyDangerousAction({ kind: 'click', element: node?.name });
  if (DESTRUCTIVE_PATTERN.test(description) || (brake && HIGH_RISK_BRAKE.has(brake.risk))) return 'high';
  if (SENSITIVE_PATTERN.test(description) || brake || action?.kind === 'type_secret') return 'medium';
  return 'low';
}
