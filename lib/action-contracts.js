...
function planAction({ action, node, snapshot, policy = {}, now = Date.now() } = {}) {
  ...
  return {
    ...
    target: { nodeId: node.id, ref: node.ref, role: node.role, name: node.name },
    action,
    risk,
...
function verifyPostconditions(postconditions = [], { url, snapshot } = {}) {
  const results = postconditions.map(condition => {
    ...
    if (condition.kind === 'url_matches') {
      try { passed = new RegExp(condition.pattern).test(url || ''); } catch { passed = false; }
    } else if (condition.kind === 'node_exists') {
      passed = snapshot?.nodes?.some(node =>
        (condition.nodeId && node.id === condition.nodeId) ||
        (condition.name && node.name === condition.name) ||
        (condition.role && node.role === condition.role && (!condition.name || node.name === condition.name))
      ) || false;
...
export { classifyActionRisk, planAction, validateContract, verifyPostconditions };
