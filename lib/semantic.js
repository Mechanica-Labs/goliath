...
const MAX_SEMANTIC_YAML_CHARS = 512_000;
const MAX_SEMANTIC_NODES = 1_000;
...
function parseAriaSnapshot(yaml = '', { mainUrl = null } = {}) {
  const nodes = [];
  const ancestry = [];
  const boundedYaml = String(yaml).slice(0, MAX_SEMANTIC_YAML_CHARS);

  for (const [lineIndex, line] of boundedYaml.split('\n').entries()) {
    ...
    const frameKey = String(attributes['frame-key'] || parent?.frameKey || 'main');
    let frameUrl = attributes['frame-url'] || parent?.frameUrl || mainUrl;
    if (attributes['frame-url']) {
      try { frameUrl = decodeURIComponent(attributes['frame-url']); } catch { frameUrl = null; }
    }
    delete attributes['frame-key'];
    delete attributes['frame-url'];
    const node = {
      index: nodes.length,
      lineIndex,
      depth,
...
function semanticKey(node, nodes, route) {
  ...
  return [route, node.frameKey, region, node.role, node.normalizedName, JSON.stringify(stableState)].join('|');
}

function matchScore(node, candidate) {
  if (node.role !== candidate.role || node.frameKey !== candidate.frameKey) return 0;
  let score = 0.25;
...
function assignSemanticIds(nodes, { url, previous } = {}) {
  ...
  const previousNodes = (previous?.nodes || []).slice(0, MAX_SEMANTIC_NODES);
  const usedPrevious = new Set();
  const collisionCounts = new Map();
  const exactPrevious = new Map();
  const rolePrevious = new Map();

  for (const candidate of previousNodes) {
    const frameKey = candidate.frameKey || candidate.provenance?.frame?.key || 'main';
    const indexedCandidate = {
      ...candidate,
      frameKey,
      normalizedName: candidate.normalizedName ?? normalizeText(candidate.name),
    };
    const exactKey = `${frameKey}|${candidate.role}|${indexedCandidate.normalizedName}`;
    const roleKey = `${frameKey}|${candidate.role}`;
    if (!exactPrevious.has(exactKey)) exactPrevious.set(exactKey, []);
    if (!rolePrevious.has(roleKey)) rolePrevious.set(roleKey, []);
    if (exactPrevious.get(exactKey).length < MAX_MATCH_CANDIDATES) exactPrevious.get(exactKey).push(indexedCandidate);
    if (rolePrevious.get(roleKey).length < MAX_MATCH_CANDIDATES) rolePrevious.get(roleKey).push(indexedCandidate);
  }
  ...
  for (const node of nodes) {
    ...
    const exactKey = `${node.frameKey}|${node.role}|${node.normalizedName}`;
    const roleKey = `${node.frameKey}|${node.role}`;
    const candidatePool = exactPrevious.get(exactKey) || rolePrevious.get(roleKey) || [];
...
function nodePublicView(node, snapshotId, url, capturedAt) {
  ...
  return {
    ...
    provenance: {
      ...
      url,
      frame: { key: node.frameKey, url: node.frameUrl || url },
...
function buildSemanticSnapshot({ yaml, url, previous = null, capturedAt = new Date().toISOString(), readiness = null } = {}) {
  ...
  const yamlText = String(yaml || '');
  const parsed = assignSemanticIds(parseAriaSnapshot(yamlText, { mainUrl: url }), { url, previous: effectivePrevious });
  const snapshotId = `snap_${shortHash(`${url}|${capturedAt}|${parsed.map(node => `${node.id}:${node.name}:${node.value}`).join('|')}`)}`;
  const snapshot = {
    snapshotId,
    stateHash: `sha256:${shortHash(`${routeTemplate(url)}|${parsed.map(node => `${node.id}:${node.frameKey}:${node.frameUrl}:${node.role}:${node.name}:${node.value}:${JSON.stringify(node.attributes)}`).join('|')}`, 32)}`,
    ...
    limits: {
      maxYamlChars: MAX_SEMANTIC_YAML_CHARS,
      maxNodes: MAX_SEMANTIC_NODES,
      truncated: yamlText.length > MAX_SEMANTIC_YAML_CHARS || parsed.length >= MAX_SEMANTIC_NODES,
    },
  };
  ...
  const memberIdsByParent = new Map();
  for (const node of snapshot.nodes) {
    if (!node.parentId) continue;
    if (!memberIdsByParent.has(node.parentId)) memberIdsByParent.set(node.parentId, []);
    memberIdsByParent.get(node.parentId).push(node.id);
  }
  snapshot.collections = snapshot.nodes
    ...
    .map(node => ({
      ...
      kind: node.role,
      name: node.name,
      memberIds: memberIdsByParent.get(node.id) || [],
    }));
