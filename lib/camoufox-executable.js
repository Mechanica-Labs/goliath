...
function shimRootFor(executablePath, resourceDir) {
  const key = crypto
    ...
    .digest('hex')
    .slice(0, 16);
  return join(tmpdir(), 'goliath-external-camoufox', key);
}
...
// Backward-compatible product-facing export.
export const prepareExternalGoliathExecutable = prepareExternalCamoufoxExecutable;
