...
// Native GOLIATH palette (frontend brand package). Hex is the source of truth.
...
export const BRAND = Object.fromEntries(
  Object.entries(BRAND_HEX).map(([name, hex]) => [name, hexToRgb(hex)]),
);

// Wordmark glow: electric purple (blacklight) into slime (the one hero source).
const GRADIENT_START = BRAND.purple;
const GRADIENT_END = BRAND.slime;
...
const UNICODE_SYMBOLS = {
  ...
  // Hard rectangle corners — brand radius is 0–2px, not rounded glass.
  tl: '┌', tr: '┐', bl: '└', br: '┘', h: '─', v: '│',
};
...
export function colorDepth(env = process.env, stream = process.stdout) {
  ...
  // iTerm and Windows Terminal speak 24-bit; use native hex instead of the 256 cube.
  if (env.TERM_PROGRAM === 'iTerm.app' || env.WT_SESSION) return 24;
...
export function createTui({ stream = process.stdout, env = process.env, plat = platform() } = {}) {
  ...
  /** Colorize with a brand RGB triple, degrading to a 16-color fallback. */
  function paint(text, [r, g, b], fallbackCode) {
    if (depth >= 24) return `${ESC}38;2;${r};${g};${b}m${text}${ESC}0m`;
    if (depth >= 8) return `${ESC}38;5;${rgbTo256(r, g, b)}m${text}${ESC}0m`;
    if (depth > 0) return wrap(fallbackCode, text);
    return String(text);
  }
  ...
  const style = {
    ...
    dim: (t) => wrap(2, t),
    italic: (t) => wrap(3, t),
    red: (t) => paint(t, BRAND.danger, 31),
    green: (t) => paint(t, BRAND.slime, 92),
    yellow: (t) => paint(t, BRAND.orange, 33),
    blue: (t) => paint(t, BRAND.cyan, 36),
    magenta: (t) => paint(t, BRAND.purple, 95),
    cyan: (t) => paint(t, BRAND.cyan, 96),
    gray: (t) => paint(t, BRAND.muted, 90),
    slime: (t) => paint(t, BRAND.slime, 92),
    purple: (t) => paint(t, BRAND.purple, 95),
    orange: (t) => paint(t, BRAND.orange, 33),
    ink: (t) => paint(t, BRAND.ink, 97),
  };

  /** Colorize with a truecolor/256 value, degrading to a plain 16-color hue. */
  function rgb(text, [r, g, b], fallback = style.green) {
    if (depth >= 8) return paint(text, [r, g, b], 92);
  ...
  function gradientText(text) {
    ...
    const last = chars.reduce((acc, ch, i) => (ch === ' ' ? acc : i), first);
    const span = Math.max((last < 0 ? 0 : last) - Math.max(first, 0), 1);
    const origin = first < 0 ? 0 : first;
    return chars.map((ch, i) => (
      ch === ' ' ? ch : rgb(ch, gradientAt((i - origin) / span))
    )).join('');
  }
  ...
  function banner(subtitle = '') {
    ...
    if (!unicode || cols < WORDMARK[0].length + 2) {
      line(`  ${gradientText('GOLIATH')}${subtitle ? style.purple(`  ${subtitle}`) : ''}`);
      line();
      return;
    }
    WORDMARK.forEach((row) => {
      line(`  ${depth === 0 ? row : gradientText(row)}`);
    });
    if (subtitle) {
      line();
      line(`  ${style.purple(subtitle)}`);
    }
    line();
  }
   ...
   */
  function box(lines, { title = '', tone = style.purple, indent = '  ' } = {}) {
  ...
  function spinner(label) {
    ...
    const render = () => {
      clearLine();
      write(`  ${style.slime(SPINNER_FRAMES[frame % SPINNER_FRAMES.length])} ${text}`);
      frame++;
    };
  ...
  function progress({ label, total = null, startedAt = Date.now() } = {}) {
    ...
    const paint = (force = false) => {
      ...
      if (total) {
        ...
        write(`  ${style.slime(SPINNER_FRAMES[Math.floor(Date.now() / 80) % SPINNER_FRAMES.length])} ${label}  ${style.gray(stats.join(`  ${sym.mid}  `))}`);
      }
    };
