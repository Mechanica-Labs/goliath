...
async function startServer(pluginDir, port, log, pluginCfg) {
    ...
    if (pluginCfg?.apiKey)
        env.GOLIATH_API_KEY = pluginCfg.apiKey;
    if (pluginCfg?.accessKey)
        env.GOLIATH_ACCESS_KEY = pluginCfg.accessKey;
    if (pluginCfg?.crashReportEnabled != null)
        env.GOLIATH_CRASH_REPORT_ENABLED = String(pluginCfg.crashReportEnabled);
    if (pluginCfg?.crashReportUrl)
        env.GOLIATH_CRASH_REPORT_URL = pluginCfg.crashReportUrl;
...
export default function register(api) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "PREFERRED: Create a Firefox-based browser tab with configurable anti-detection measures and human interaction support. Acceptance by third-party bot-detection systems is not guaranteed. Returns tabId for subsequent operations.",
        parameters: {
            type: "object",
            properties: {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Get accessibility snapshot of a Goliath page with element refs (e1, e2, etc.) for interaction, plus a visual screenshot. " +
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Navigate a Goliath tab to a URL or use a search macro (@google_search, @youtube_search, etc.). Preferred over Chrome for sites with bot detection.",
        parameters: {
            type: "object",
            properties: {
                tabId: { type: "string", description: "Tab identifier" },
    ...
    api.registerTool((ctx) => ({
        name: "goliath_hands",
        description: "Run an ordered multi-step UI workflow (a 'hand') in one Goliath tab. Actions: click, type, select, check, wait, scroll, press, submit. Stops at the first failing step and returns per-step results plus failedStep. Prefer this over repeated click/type calls when filling multi-field forms.",
        parameters: {
            type: "object",
            properties: {
                tabId: { type: "string", description: "Tab identifier" },
                steps: {
                    type: "array",
                    description: "Ordered UI steps (max 20). Target with `ref` from goliath_snapshot or a CSS `selector`. `select` requires a CSS selector.",
                    items: {
                        type: "object",
                        required: ["action"],
                        properties: {
    ...
    api.registerTool((ctx) => ({
        name: "goliath_upload",
        description: "Attach one or more files to a page upload control. Files must be absolute server-side paths inside GOLIATH_UPLOADS_DIR.",
        parameters: {
            type: "object",
            properties: {
                tabId: { type: "string", description: "Tab identifier" },
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Execute JavaScript in a Goliath tab's page context. Returns the result of the expression. Use for injecting scripts, reading page state, or calling web app APIs.",
        parameters: {
            type: "object",
            properties: {
                tabId: { type: "string", description: "Tab identifier" },
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Import cookies into the current Goliath user session (Netscape cookie file). Use to authenticate to sites like LinkedIn without interactive login.",
        parameters: {
            type: "object",
            properties: {
        ...
        async execute(_id, params) {
            ...
            if (!envCfg.apiKey) {
                throw new Error("GOLIATH_API_KEY is not set. Cookie import is disabled unless you set GOLIATH_API_KEY for both the server and the OpenClaw plugin environment.");
            }
