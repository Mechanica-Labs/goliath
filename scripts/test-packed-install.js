...
try {
  ...
  if (manifest.name !== '@mechanica-labs/goliath' || manifest.version !== expectedVersion || manifest.private !== false) {
  ...
  if (versionOutput !== expectedVersion) throw new Error(`packed CLI reported ${versionOutput}`);
  ...
  if (!generic.args.includes(`@mechanica-labs/goliath@${expectedVersion}`)) throw new Error('packed harness config has the wrong npm identity');
