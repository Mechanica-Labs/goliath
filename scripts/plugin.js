...
/**
 * goliath plugin manager -- install, remove, and list plugins.
 *
 * Usage:
 ...
 *
 * After install, the plugin is added to goliath.config.json plugins[] and
...
const CONFIG_PATH = path.join(ROOT, 'goliath.config.json');
...
function readConfig() {
  try {
    ...
    return { id: 'goliath', name: 'Goliath', version: '0.0.0', plugins: [] };
  }
}
...
function listPlugins() {
  ...
  if (configPlugins.size > 0) {
    console.log(`\n${configPlugins.size} plugin(s) enabled in goliath.config.json`);
  } else {
...
switch (action) {
  ...
  default:
    console.log(`goliath plugin manager
